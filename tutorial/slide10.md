...
두 모델 전부 RNN 계열의 LSTM 이나 GRU를 사용하며 
...
문장이나 단어를 seq로 전처리 할때 끝 부분에 <END> 토큰을 추가해서 하거나 혹은 모델 자체에서 처리하도록 만들수있다.    
pytorch 예제의 경우 토크나이징은 전부 one-hot으로 처리했으나, embedding 모델도 처리할 수 있다.
